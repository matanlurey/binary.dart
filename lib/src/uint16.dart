...
extension type const Uint16._(int _) implements Comparable<num> {
  ...
  /// Uint16(8).log(2); // 3
  /// ```
  Uint16 log([int base = 10]) => Uint16.fromUnchecked(_.log(base));
  ...
  Uint16 uncheckedSetNthBit(int n, [bool value = true]) {
    return Uint16.fromUnchecked(_.setNthBit(n, value));
  }
  ...
  Uint16 uncheckedToggleNthBit(int n) {
    return Uint16.fromUnchecked(_.toggleNthBit(n));
  }
  ...
  ///
  /// If either [left] or [right] is out of range, the behavior is undefined.
  Uint16 uncheckedBitReplace(int value, int left, [int? right]) {
