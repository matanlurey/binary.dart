...
extension type const Int8._(int _) implements Comparable<num> {
  ...
  /// Int8(8).log(2); // 3
  /// ```
  Int8 log([int base = 10]) => Int8.fromUnchecked(_.log(base));
  ...
  Int8 uncheckedSetNthBit(int n, [bool value = true]) {
    return Int8.fromUnchecked(_.setNthBit(n, value));
  }
  ...
  Int8 uncheckedToggleNthBit(int n) {
    return Int8.fromUnchecked(_.toggleNthBit(n));
  }
  ...
  ///
  /// ```dart
  /// Int8(3).countLeadingOnes(); // <width - 2>
  /// ```
  ...
  ///
  /// If either [left] or [right] is out of range, the behavior is undefined.
  Int8 uncheckedBitReplace(int value, int left, [int? right]) {
    return _descriptor.uncheckedBitReplace(_, value, left, right);
  }
