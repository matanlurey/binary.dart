...
import 'package:binary/binary.dart'
    show FixedInt, Uint8, debugCheckFixedWithInRange;
...
void main() {
  ...
  test('isValid, checkRange', () {
    check(Uint8.isValid(0)).isTrue();
    check(Uint8.isValid(255)).isTrue();
    check(Uint8.isValid(-1)).isFalse();
    check(Uint8.isValid(256)).isFalse();

    check(() => Uint8.checkRange(0)).returnsNormally();
    check(() => Uint8.checkRange(255)).returnsNormally();
    check(() => Uint8.checkRange(-1)).throws<Error>();
    check(() => Uint8.checkRange(256)).throws<Error>();
  });
  ...
  group('Int8.fromInt', () {
    test('same number', () {
      ...
      check(Uint8.fromInt($F)).equals(Uint8(0xF));
    });

    test('truncates', () {
      final $FFF = 0xFFF as FixedInt;
      check(Uint8.fromInt($FFF)).equals(Uint8.max);
    });
  });
  ...
  group('bit slice and range operations', () {
    test('should return the last 4 bits, left-padded with 0s', () {
      ...
      //                    ^^^^
      final s1 = Uint8(i1).chunk(4).toBinaryString();
    ...
    test('should return the first 4 bits', () {
      final i1 = int.parse('01111111', radix: 2);
      //                        ^^^^
      final s1 = Uint8(i1).chunk(0, 4).toBinaryString();
    ...
    test('should return the middle 4 bits', () {
      final i1 = int.parse('01111111', radix: 2);
      //                      ^^^^
      final s1 = Uint8(i1).chunk(2, 4).toBinaryString();
      check(s1).equals('00001111');
    });
    ...
    test('should use slice to get the last 4 bits', () {
      final i1 = int.parse('01111111', radix: 2);
      //                    ^^^^
      final s1 = Uint8(i1).slice(4).toBinaryString();
      check(s1).equals('00000111');
    });
    ...
    test('should use slice to get the first 4 bits', () {
      final i1 = int.parse('01111111', radix: 2);
      //                        ^^^^
      final s1 = Uint8(i1).slice(0, 3).toBinaryString();
      check(s1).equals('00001111');
    });
    ...
    test('should use slice to get the middle 4 bits', () {
      final i1 = int.parse('01111111', radix: 2);
      //                      ^^^^
      final s1 = Uint8(i1).slice(2, 5).toBinaryString();
      check(s1).equals('00001111');
    });
    ...
    test('should replace the first 4 bits with 1010', () {
      ...
      final s1 = Uint8(i1).replace(0, 3, i2).toBinaryString();
    ...
    test('should replace the last 4 bits with 1010', () {
      ...
      //                    ^^^^
      final i2 = int.parse('1010', radix: 2);
      final s1 = Uint8(i1).replace(4, null, i2).toBinaryString();
  ...
  test('operator ^', () {
    check(Uint8(0x55) ^ Uint8(0x33)).equals(Uint8(0x66));
  });
}
